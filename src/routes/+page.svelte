<script lang="ts">
    ...
    let { locked, cover } = data;
</script>

<main class="home" class:locked>
    {#if locked}
        <form class="challenge passcode" autoComplete="off" method="POST">
            <h1>Listening party</h1>
            <p>Enter the passcode to unlock all content:</p>
            <div class="input rewards">
                <input
                    type="text"
                    id="rewards-email"
                    name="passcode-entry"
    ...
    {:else}
        ...
        <div class="home-as">
            ...
            <div class="a-buttons">
                {#if Date.now() > 1667509200000}
                    <a role="button" target="_blank" href="https://ditto.fm/man-of-substance" rel="noreferrer">
                        Stream the album
                    </a>
                {/if}
                <a role="button" href="/contributors">
                    Contributors
                </a>
                <a role="button" href="/tracks">
                    Tracklist
                </a>
                <a role="button" href="/credits">
                    Album credits
                </a>
